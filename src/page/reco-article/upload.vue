<template>
    <div id="Content-page">
        <Paging :pagData="[{name:'推荐文章',router:'/reco-article'},{name:'选择文章',router:'/reco-article-upload'}]" />
            ...
            <div class="global-display global-j-content">
                ...
                <div class="right-upload-button">
                    <el-button type="success" class="submit-button" :loading="loadIng" @click="subMit">
                        {{ recoArticleId == '' ? '提交' : '确认修改' }}
                    </el-button>
                </div>
            </div>
            <!-- 已选择文章 -->
           <div v-if="tableSelect.length > 0">
            <p class="select-Title">已选择文章</p>
            ...
            <!-- 选择文章 -->
            <p class="select-Title">请选择一个文章关联</p>
...
<script setup>
    ...
    async function requestApi(page=1){
        const res = await request.get('/allUserArticle',{page})
        console.log(res);
    ...
    function handleCurrentChange(event){
        ...
        articleId.value = event ? event._id : articleId.value
    }
    // 提交
    async function subMit(){
    try {
        loadIng.value = true
        if(recoArticleId.value == ''){
            await request.post('/uploadFourArticle',{
                imageUrl:uploadImgurl.value,
                article_id:articleId.value,
            })
        }else{
            await request.post('/modifyRecomArticle',{
                _id:recoArticleId.value,
                imageUrl:uploadImgurl.value,
                article_id:articleId.value,
            })
        }
        $router.push('/reco-article')
        loadIng.value = false
    } catch (error) {
        loadIng.value = false
    }
